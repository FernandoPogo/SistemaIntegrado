...
if(isset($_POST["NuevoComentario"]))
{
  ...
  $StoreProcedure ='CALL sp_RegistrarComentario ('.$NewCodigo.',\''.$NewUsuario.'\',\''.$NewOpinion.'\',@v_Mensaje);
';
  echo "<script>alert(".$StoreProcedure.");</script>";
  
  $q=$conexion->prepare($StoreProcedure);
  $q->execute();
  print($StoreProcedure);
  echo '<script>alert('.$StoreProcedure.');</script>';
  ...
  unset($_POST["NuevoComentario"]);
  unset($_POST["CodigoForo"]);
   
  header( 'Location: ../Comunidad Desarrollo/Buscar.php' );
  return;
}
else
{
  echo "<script>alert('"."Estoy por aqui"."');</script>";
                ...
                echo '<div class="container">';
                  ...
                  echo '<form method="post" class="top5" enctype="multipart/form-data">';
                    echo '<div class="form-group">';
                      echo '<textarea name="NuevoComentario" id="NuevoComentario" rows="3" class="form-control"  placeholder="Ingrese un comentario" required></textarea>';
                    echo '</div>';
                    echo '<div class="form-group">';
